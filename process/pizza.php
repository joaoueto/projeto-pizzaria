<?php
  ...
  // Criação do pedido
    ...
    if(count($sabores) > 3) {
      ...
      $stmt = $conn->prepare("INSERT INTO pizzas_sabor (pizza_id, sabor_id) VALUES (:pizzas, :sabor)");

      // repetição até terminar de salvar todos os sabores
      foreach($sabores as $sabor) {

        // filtrando os inputs
        $stmt->bindParam(":pizzas", $pizzaId, PDO::PARAM_INT);
      ...
      $stmt = $conn->prepare("INSERT INTO pedidos (pizza_id, status_id) VALUES (:pizzas, :status)");

      // status -> sempre inicia com 1, que é em produção
      $statusId = 1;

      // filtrar inputs
      $stmt->bindParam(":pizzas", $pizzaId);
